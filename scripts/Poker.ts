...
export class Poker extends Ruleset {
    ...
    dealHoldem = (players:Player[],  deck:Deck, river:Card[] = []) : {players:Player[],hands:Card[][],river:Card[]}=> {
        ...
        for (let i = 0; i < players.length; i++) {
            let P = players[i];
            if (P.hand.length > 0) deck.shuffleIn(P.hand);
            hands[i] = P.hand;
        }
        deck.dealRound(hands,2);
        deck.dealRound([river],5);

        return {players:players, hands:hands, river:river};
        // res = {
        //     players:Array.from(players),
        //     hands:Array.from(hands),
        //     river:Array.from(river)
        // }

        // for (let player of players) deck.shuffleIn(player.hand);
        // deck.shuffleIn(river);
...
// const Poker = new PokerRuleset();
// Poker.deal = (players:Player[], deck:Deck) => {
//     return Poker.dealHoldem(players,deck);
// }